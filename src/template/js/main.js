...
(({
    ...
    $(document).ready(() => {
        ...
        $("#respawn").click(() => { // respawn function
            socket.emit("room respawn");
            $("#respawn").fadeOut(200);
        });

        $("#chat").keypress(e => { // chatting
            if (e.which == 13) {
        ...
        $("#refresh").click(() => { // refresh room
            socket.emit("Refresh");
        });

        $("#rename").click(async () => { // change name
    ...
    const showSketch = utils.showSketch = (timer) => {
        $("#menu").fadeOut(timer, "", () => {
            $("#menu").hide();
    ...
    const showMenu = utils.showMenu = (timer) => {
